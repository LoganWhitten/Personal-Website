...
// node_modules/@react-spring/parallax/dist/react-spring_parallax.modern.mjs
var ParentContext = React3.createContext(null);
...
function mapChildrenRecursive(children, callback) {
  const isReactFragment = (node) => {
    if (node.type) {
      return node.type === React3.Fragment;
    }
    return node === React3.Fragment;
  };
...
var ParallaxLayer = React3.memo(
  React3.forwardRef(
    ({ horizontal, factor = 1, offset = 0, speed = 0, sticky, ...rest }, ref) => {
      const parent = (0, import_react15.useContext)(ParentContext);
      const ctrl = useMemoOne(() => {
        let translate;
        if (sticky) {
          const start2 = sticky.start || 0;
          translate = start2 * parent.space;
        } else {
      ...
      useOnce(() => {
        if (sticky) {
          const start2 = sticky.start || 0;
          const end = sticky.end || start2 + 1;
          layer.sticky = { start: start2, end };
        }
      });
      React3.useImperativeHandle(ref, () => layer);
      const layerRef = (0, import_react15.useRef)();
      const setSticky = (height, scrollTop) => {
        const start2 = layer.sticky.start * height;
        const end = layer.sticky.end * height;
        const isSticky = scrollTop >= start2 && scrollTop <= end;
        ...
        ctrl.set({
          translate: isSticky ? 0 : scrollTop < start2 ? start2 : end
        });
      };
      useOnce(() => {
        if (parent) {
...
var Parallax = React3.memo(
  React3.forwardRef((props, ref) => {
    ...
    const containerRef = (0, import_react15.useRef)();
    const contentRef = (0, import_react15.useRef)();
    ...
    React3.useImperativeHandle(ref, () => state);
    const update3 = () => {
    ...
    (0, import_react15.useEffect)(() => state.update());
    useOnce(() => {
      setReady(true);
      const onResize2 = () => {
        const update22 = () => state.update();
        raf.onFrame(update22);
        setTimeout(update22, 150);
      };
      window.addEventListener("resize", onResize2, false);
      return () => window.removeEventListener("resize", onResize2, false);
    });
    ...
    return React3.createElement(
      ...
      ready && React3.createElement(React3.Fragment, null, React3.createElement(
        animated.div,
        {
          ref: contentRef,
          style: {
        ...
        React3.createElement(ParentContext.Provider, { value: state }, mapChildrenRecursive(
          children,
